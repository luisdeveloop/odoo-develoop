...
class AccountInvoiceReport(models.Model):
    ...
    def init(self):
        ...
        data = """CREATE or REPLACE VIEW %s as (
            SELECT MIN(a.id) as id, MIN(a.number) AS number, MIN(a.date) AS date, MIN(a.product_id) product_id, MIN(a.partner_id) partner_id, MIN(a.country_id) country_id,
                   MIN(a.account_analytic_id) account_analytic_id, MIN(a.payment_term_id) payment_term_id,  MIN(a.uom_name) uom_name, MIN(a.currency_id) currency_id,
                   MIN(a.journal_id) journal_id, MIN(a.fiscal_position_id) fiscal_position_id, MIN(a.user_id) user_id, MIN(a.company_id) company_id, MIN(a.nbr) nbr,
                   a.invoice_id, MIN(a.type) AS type, MIN(a.state) state, MIN(a.categ_id) categ_id, MIN(a.date_due) date_due, MIN(a.account_id) account_id,
                   MIN(a.account_line_id) account_line_id, MIN(a.partner_bank_id) partner_bank_id, MIN(a.product_qty) product_qty, MIN(a.price_total) price_total,
                   MIN(a.price_average) price_average, MIN(a.amount_total) amount_total, MIN(a.currency_rate) currency_rate, MIN(a.residual) residual, MIN(a.commercial_partner_id) commercial_partner_id
            FROM (
